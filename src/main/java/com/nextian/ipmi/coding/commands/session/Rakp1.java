...
import com.nextian.ipmi.common.Randomizer;
...
public class Rakp1 extends IpmiCommandCoder {
    ...
    /**
     * Requested command privilege level as defined by {{@link PrivilegeLevel}}
     */
     ...
     *
     * @param managedSystemSessionId the Managed System's Session ID for this session. Must be as
     ...
     *                               being used ({@link GetChannelAuthenticationCapabilitiesResponseData#isKgEnabled()}
     *                               == false)
     * @param cipherSuite            {@link CipherSuite} containing authentication, confidentiality and integrity
     *                               algorithms for this session.
     */
    public Rakp1(int managedSystemSessionId, PrivilegeLevel privilegeLevel, String username, String password,
                 byte[] bmcKey, CipherSuite cipherSuite) {
    ...
    protected IpmiPayload preparePayload(int sequenceNumber) {
        ...
        byte[] sId = TypeConverter.intToLittleEndianByteArray(getManagedSystemSessionId());
        ...
        payload[24] = TypeConverter.intToByte(encodePrivilegeLevel(requestedMaximumPrivilegeLevel) | 0x10);
        ...
        if (getUsername().length() > 0) {
            System.arraycopy(getUsername().getBytes(), 0, payload, 28, getUsername().length()); // username
        }
    ...
    /**
     * Get response data from IPMI message
     *
     * @throws IllegalArgumentException when message is not a response for class-specific command, response has invalid
     *                                  length or authentication check fails
    ...
    public ResponseData getResponseData(IpmiMessage message)
        ...
        int length = getCipherSuite().getAuthenticationAlgorithm().getKeyLength();
        ...
        if (!getCipherSuite().getAuthenticationAlgorithm()
                .checkKeyExchangeAuthenticationCode(prepareKeyExchangeAuthenticationCodeBase(data), key, getPassword())) {
    ...
    /**
     * @return byte array holding prepared base for calculating KeyExchangeAuthenticationCode for RAKP Message 2
     */
    private byte[] prepareKeyExchangeAuthenticationCodeBase(Rakp1ResponseData responseData) {
        ...
        byte[] mSID = TypeConverter.intToLittleEndianByteArray(getManagedSystemSessionId());
        ...
        keac[56] = TypeConverter.intToByte(encodePrivilegeLevel(requestedMaximumPrivilegeLevel) | 0x10);

        if (getUsername() != null) {
            keac[57] = TypeConverter.intToByte(getUsername().length());
            if (getUsername().length() > 0) {
                System.arraycopy(getUsername().getBytes(), 0, keac, 58, getUsername().length());
            }
        } else {
            keac[57] = 0;
        }
     ...
     *
     * @param responseData RAKP Message 2 data
    ...
    public byte[] calculateSik(Rakp1ResponseData responseData) throws InvalidKeyException, NoSuchAlgorithmException {
        ...
        return getCipherSuite().getAuthenticationAlgorithm()
                .getKeyExchangeAuthenticationCode(prepareSikBase(responseData), new String(key));
    }

    /**
     * @return byte array holding prepared base for calculating Session Integrity Key
     */
    private byte[] prepareSikBase(Rakp1ResponseData responseData) {
        ...
        sikBase[32] = TypeConverter.intToByte(encodePrivilegeLevel(requestedMaximumPrivilegeLevel) | 0x10);

        if (getUsername() != null) {
            sikBase[33] = TypeConverter.intToByte(getUsername().length());
            if (getUsername().length() > 0) {
                System.arraycopy(getUsername().getBytes(), 0, sikBase, 34, getUsername().length());
            }
        } else {
            sikBase[33] = 0;
        }
