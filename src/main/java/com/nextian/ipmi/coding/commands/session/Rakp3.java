...
/**
 * A wrapper for RMCP+ RAKP3 message and it's response - RAKP4 message. It extends RAKP1 message.
 */
public class Rakp3 extends IpmiCommandCoder {
    ...
    /**
     * The Managed System's Session ID for this session. Must be as returned by the Managed System in the
     * Open Session Response message.
     */
    private int managedSystemSessionId;

    /**
     * Message content included in base message RAKP1
     */
    private Rakp1 rakp1;

    /**
     * Response content as specified by RAKP1 message
     */
    ...
    /**
     * Initiates class for decoding. Sets IPMI version to {@link IpmiVersion#V20} since RAKP1 is a RMCP+ command. Sets
     ...
     *                          for this session. Only authentication algorithm is used at this point of
     *                          creating a session
     * @param rakp1             RAKP Message 1 sent earlier in the authentication process
    ...
    /**
     * Initiates class for encoding and decoding. Sets IPMI version to {@link IpmiVersion#V20} since RAKP1 is a RMCP+
     * command. Sets Authentication Type to RMCP+.
     *
     ...
     *                               algorithms for this session. Only authentication algorithm is used at this point
     *                               of creating a session
     * @param rakp1                  RAKP Message 1 sent earlier in the authentication process
     * @param rakp1ResponseData      RAKP Message 2 received earlier in the authentication process
     */
    public Rakp3(byte statusCode, int managedSystemSessionId, CipherSuite cipherSuite, Rakp1 rakp1,
    ...
    public IpmiMessage encodeCommand(int sequenceNumber, int sessionId)
        ...
        message.setPayloadAuthenticated(getCipherSuite().getIntegrityAlgorithm().getCode() != SecurityConstants.IA_NONE);
        message.setPayloadEncrypted(false);

        message.setPayload(preparePayload(sequenceNumber));

        message.setAuthCode(getCipherSuite().getIntegrityAlgorithm()
    ...
    protected IpmiPayload preparePayload(int sequenceNumber) throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        byte[] manSesId = TypeConverter.intToLittleEndianByteArray(getManagedSystemSessionId());

        System.arraycopy(manSesId, 0, payload, 4, 4); // managed system session
        // id

        byte[] exchangeAuthCode = getCipherSuite().getAuthenticationAlgorithm().getKeyExchangeAuthenticationCode(
                prepareKeyExchangeAuthenticationCodeBase(rakp1, rakp1ResponseData), rakp1.getPassword());
    ...
    /**
     * @return byte array holding prepared base for calculating KeyExchangeAuthenticationCode for RAKP Message 3
     */
    private byte[] prepareKeyExchangeAuthenticationCodeBase(Rakp1 rakp1, Rakp1ResponseData responseData) {
        ...
        System.arraycopy(TypeConverter.intToLittleEndianByteArray(responseData.getRemoteConsoleSessionId()),
                0, keac, 16, 4);

        keac[20] = TypeConverter.intToByte(encodePrivilegeLevel(rakp1.getRequestedMaximumPrivilegeLevel()) | 0x10);
    ...
    public ResponseData getResponseData(IpmiMessage message)
            throws IllegalArgumentException, IPMIException, InvalidKeyException, NoSuchAlgorithmException {
        ...
        if (payload.length > 8) {
            integrityCheck = new byte[getCipherSuite().getAuthenticationAlgorithm().getIntegrityCheckBaseLength()];
            System.arraycopy(payload, 8, integrityCheck, 0,
                    getCipherSuite().getAuthenticationAlgorithm().getIntegrityCheckBaseLength());
        }

        if (!getCipherSuite().getAuthenticationAlgorithm().doIntegrityCheck(prepareIntegrityCheckBase(
                rakp1, rakp1ResponseData), integrityCheck, rakp1.calculateSik(rakp1ResponseData))) {
