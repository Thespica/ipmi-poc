...
public class Protocolv15Encoder extends ProtocolEncoder {
    ...
    /**
     * @param ipmiMessage IPMI message to be encoded. Must be {@link Ipmiv15Message}.
     * @throws InvalidKeyException      when initiation of the confidentiality algorithm fails
     * @throws IllegalArgumentException when IPMI protocol version is incorrect.
     * @see Ipmiv15Message
     */
    @Override
    ...
    private int getMessageLength(IpmiMessage ipmiMessage) {
        int length = 11
                + ipmiMessage.getConfidentialityAlgorithm().getConfidentialityOverheadSize(
     ...
     *
     * @param authCode raw authentication code
     ...
     */
    private void encodeAuthenticationCode(byte[] authCode, byte[] message, int offset) throws IndexOutOfBoundsException {
     ...
     * @throws IndexOutOfBoundsException when message is too short to hold value at given offset
     */
    private int encodeSessionTrailer(byte[] message, int offset) throws IndexOutOfBoundsException {
