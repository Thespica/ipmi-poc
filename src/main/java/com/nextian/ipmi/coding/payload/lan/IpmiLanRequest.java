...
public class IpmiLanRequest extends IpmiLanMessage {
     ...
     *
     * @param networkFunction command specific {@link NetworkFunction}
     * @param commandCode     command specific {@link CommandCodes}
     * @param requestData     command specific payload
     * @param sequenceNumber  used to match request and response - must be in range <0-63>
     */
     ...
     * @param requestData     command specific payload
     * @param sequenceNumber  used to match request and response - must be in range <0-63>
     * @param lun             target Logical Unit Number. Must be in range <0-3>.
     */
    ...
    public byte[] getPayloadData() {
        ...
        message[1] = TypeConverter.intToByte((networkFunction << 2) | getResponderLogicalUnitNumber());
        message[2] = getChecksum1(message);
        message[3] = getRequesterAddress();
        message[4] = TypeConverter.intToByte(((getSequenceNumber() & 0x3f) << 2) | getResponderLogicalUnitNumber());
