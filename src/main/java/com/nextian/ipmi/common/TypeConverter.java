...
public final class TypeConverter {
     ...
     * This function casts int to pseudo unsigned byte.
     *
    ...
    /**
     * Because Java doesn't support unsigned byte values, simple type casting leads to overflows and wrong values.
     * This function casts pseudo unsigned byte to int.
     *
    ...
    /**
     * Converts BCD encoded byte with bits 7:4 holding the Least Significant digit of the revision and bits 3:0
    ...
    public static String decodeBcdPlus(byte[] text) {
        ...
        for (int i = 0; i < text.length; ++i) {
            result[2 * i] = decodeBcdPlusChar(TypeConverter.intToByte((TypeConverter.byteToInt(text[i]) & 0xf0) >> 4));
            result[2 * i + 1] = decodeBcdPlusChar(TypeConverter.intToByte(TypeConverter.byteToInt(text[i]) & 0xf));
        }
